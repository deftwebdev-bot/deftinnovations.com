...
import { TrustedBrand, FALLBACK_BRANDS, getMediaUrl } from "@/lib/api";
...
export const ClientsSection: React.FC<ClientsSectionProps> = ({
  ...
  const brandList = brands && brands.length > 0 ? brands : FALLBACK_BRANDS;

  // Placeholder brands if few exist
  const placeholders = [
    { id: 101, name: "NEXUS CAPITAL", logoUrl: null },
    { id: 102, name: "AURA LUXURY", logoUrl: null },
    { id: 103, name: "SENTINEL TECH", logoUrl: null },
    { id: 104, name: "VORTEX ENERGY", logoUrl: null },
    { id: 105, name: "LUMEN HEALTH", logoUrl: null },
    { id: 106, name: "NOVA RETAIL", logoUrl: null },
    { id: 107, name: "HYPER SCALE", logoUrl: null },
    { id: 108, name: "PRISM MEDIA", logoUrl: null },
    { id: 109, name: "QUANTUM AI", logoUrl: null },
    { id: 110, name: "ATELIER GLOBAL", logoUrl: null },
  ];

  const displayList = brandList.length >= 6 ? brandList : [...brandList, ...placeholders.slice(0, 10 - brandList.length)];

  return (
    <section className="section bg-[#ffffff] text-[#0a0a0a] border-b border-black/[0.06]">
      <div className="container-xl space-y-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-end pb-6 border-b border-black/[0.08]">
          ...
          <div className="lg:col-span-7 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            ...
            <Link
              href="/portfolio"
              className="btn-outline-dark text-sm shrink-0 inline-flex items-center gap-2 group"
            >
        ...
        {/* 5-Col Client Logo Grid (WAC Style) */}
          ...
          {displayList.map((brand, idx) => {
            ...
            return (
              <FadeIn key={brand.id || idx} direction="up" delay={idx * 0.04}>
                <div className="h-28 sm:h-32 rounded-2xl bg-[#f8f9fa] border border-black/[0.06] hover:border-black/20 hover:bg-white hover:shadow-md transition-all duration-300 flex items-center justify-center p-6 text-center group">
                  {hasValidLogo ? (
